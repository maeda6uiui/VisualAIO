...
config=BertConfig.from_json_file("../PretrainedModel/KyotoUniv/bert_config.json")
tokenizer = BertJapaneseTokenizer.from_pretrained(
    "../PretrainedModel/KyotoUniv/vocab.txt",do_lower_case=False,do_basic_tokenize=False)
...
def convert_examples_to_features_text_only(examples,context_dict,option_num,max_seq_length):
    ...
    for example_index,example in enumerate(tqdm(examples)):
        ...
        for i,ending in enumerate(example.endings):
            ...
            #Create input ids.
            input_ids_text_a=tokenizer.convert_tokens_to_ids(tokenized_text_a)
            input_ids_text_a_length=len(input_ids_text_a)
            input_ids_text_a=torch.tensor(input_ids_text_a)

            input_ids_text_b=tokenizer.convert_tokens_to_ids(tokenized_text_b)
            ...
            input_ids_tmp=torch.cat([input_ids_text_a,input_ids_text_b],dim=0)
            input_ids_tmp=input_ids_tmp[:max_seq_length]
...
def main(do_train,train_batch_size,train_epoch_num,model_filename,result_save_dir):
    ...
    model = BertForMultipleChoice.from_pretrained(
        "../PretrainedModel/KyotoUniv/pytorch_model.bin",config=config
    )
    if torch.cuda.is_available():
        model.cuda()
